...
import { TabAuthorCanceledTasks } from "../TabAuthorCanceledTasks/TabAuthorCanceledTasks"
...
export function AuthorCanceledTasks({ data }: Props) {
                ...
                <tbody>
                    {data.map((item, index) => (
                        <tr key={index} className="h-20 border-t-1 border-b-1 border-gray-300">
                            <TabAuthorCanceledTasks data={item} executor={executors ? executors.filter(executor => executor.id === item.executorid) : null} status={statuses ? statuses.filter(status => status.id === String(item.statusid)) : null}/>
                        </tr>
                    ))}
                </tbody>
            </table>
