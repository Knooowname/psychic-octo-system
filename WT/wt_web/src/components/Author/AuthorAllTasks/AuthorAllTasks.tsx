...
import { TabAuthorAllTasks } from "../TabAuthorAllTasks/TabAuthorAllTasks"
...
export function AuthorAllTasks({ data }: Props) {
            ...
            <table className="min-w-max table-auto border-collapse box-border">
                <thead>
                    <tr className="bg-gray-100">
                        ...
                        <td className="border border-white p-2 w-48 text-left">Документы</td>
                ...
                <tbody>
                    {data.map((item, index) => (
                        <tr key={index} className="h-20 border-t-1 border-b-1 border-gray-300">
                            <TabAuthorAllTasks data={item} executor={executors ? executors.filter(executor => executor.id === item.executorid) : null} priority={prioritys ? prioritys.filter(priority => priority.id === String(item.priorityid)) : null} status={statuses ? statuses.filter(status => status.id === String(item.statusid)) : null} />
                        </tr>
                    ))}
                </tbody>
            </table>
