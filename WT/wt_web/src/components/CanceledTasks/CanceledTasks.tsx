...
import { TabCanceledTask } from "../TabCanceledTask/TabCanceledTask";
...
export function CanceledTasks({dataArr}: Props) {
                ...
                <tbody>
                    {dataArr?.map((item, index) => (
                        <tr key={index} className="h-20 border-t-1 border-b-1 border-gray-300">
                             <TabCanceledTask status={statuses ? statuses.filter(status => status.id === String(item.statusid)) : null} author={authors ? authors.filter(author => author.id === item.authorid) : null} data={item}/>
                        </tr>
                    ))}
                </tbody>
              </table>
