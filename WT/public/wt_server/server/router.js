...
function Router(body) {
    return __awaiter(this, void 0, void 0, function () {
        var res, data, _a, user, user, user_sess_code, user, user, user, task, task, task, task, task, status, priority, task;
            ...
            switch (_b.label) {
                case 0:
                    ...
                    switch (_a) {
                        ...
                        case "auth_code": return [3, 9];
                        case "recover_pass": return [3, 14];
                        case "get_users": return [3, 16];
                        case "add_task": return [3, 18];
                        case "get_executor_task": return [3, 20];
                        case "get_author_task": return [3, 22];
                        case "update_task": return [3, 24];
                        case "update_task_author": return [3, 26];
                        case "get_status": return [3, 28];
                        case "get_priority": return [3, 30];
                        case "update_task_priority": return [3, 32];
                    }
                    return [3, 34];
                case 1:
                    {
                ...
                case 4:
                    user = new User_1.User(body.args);
                    return [4, user.insertSessionCode()];
                case 5:
                    user_sess_code = _b.sent();
                    if (!(user_sess_code === undefined)) return [3, 6];
                    res.cmd = body.cmd;
                    res.error =
                    ...
                    res.user_sess_code = "";
                    return [3, 8];
                case 6: return [4, user.selectUser()];
                case 7:
                    data = _b.sent();
                    res.cmd = body.cmd;
                    res.error = null;
                ...
                case 10:
                    data = _b.sent();
                    if (!(data.length === 0 || data[0] === undefined)) return [3, 12];
                    res.cmd = body.cmd;
                    res.error =
                        "Данного кода сессии не существует или закончилось время использования кода";
                    res.data = null;
                    res.user_sess_code = "";
                    return [4, user.deleteSessionCode()];
                case 11:
                    _b.sent();
                    return [3, 13];
                case 12:
                ...
                case 34:
                    ...
                    _b.label = 35;
                case 35: return [2, JSON.stringify(res)];
            }
        });
    });
}
